...
public class JwtUtil {
    ...
    public boolean isTokenExpired(String token) {
        if (!StringUtils.hasLength(token)) {
            log.debug(ErrorDetail.builder().errorType(ErrorType.TOKEN_NOT_PROVIDED).severity(Severity.FATAL).build().toString());
            return true;
        }

        var claims = parseToken(token);
        var isExpired =claims.getExpiration().before(new Date());
        log.debug("{}", isExpired ? ErrorDetail.builder().errorType(ErrorType.TOKEN_EXPIRED).severity(Severity.FATAL).build().toString() : "");
        return isExpired;
    }
